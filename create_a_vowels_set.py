...
if __name__ == '__main__':
    ...
    train_recordings_rek = tuple(filter(re.compile(r'\d+_a').findall, map(str, train_recordings_rek)))
    test_recordings_rek = tuple(filter(re.compile(r'\d+_a').findall, map(str, test_recordings_rek)))
    ...
    train_recordings = tuple(filter(re.compile(r'\d+_a').findall, map(str, train_recordings)))
    test_recordings = tuple(filter(re.compile(r'\d+_a').findall, map(str, test_recordings)))
    assert not tuple(filter(test_recordings.__contains__, train_recordings))
    Path('Data/Lists/Vowels_a_Rekurrensparese_train.txt').write_text('\n'.join(f"{path} 1" for path in train_recordings_rek) + '\n' + '\n'.join(f"{path} 0" for path in train_recordings))
    Path('Data/Lists/Vowels_a_Rekurrensparese_test.txt').write_text('\n'.join(f"{path} 1" for path in test_recordings_rek) + '\n' + '\n'.join(f"{path} 0" for path in test_recordings))
